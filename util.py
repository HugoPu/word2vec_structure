...
class UpdateCollection(object):
    ...
    def update_snapshot_name(self, var_coll_name):
        ...
        for i, value in enumerate(var_list.bytes_list.value):
            ...
            var_def.ParseFormString(value)
            # Somehow node Model/global_step/read doesn't have any fanout seems to
    ...
    def replicate_states(self, state_coll_name):
        ...
        for replica_id in range(num_states):
        ...
        for replica_id in range(FLAGS.num_gpus):
            for i in range(num_states):
                ...
                state_list.node_list.value[index] = \
                    with_autoparallel_prefix(replica_id, state_list.node_list.value[index])
...
def auto_parallel(metagraph, model):
    ...
    rewriter_config.auto_parallel.num_replicas = FLAGS.num_gpups
    optimized_graph = tf_optimizer.OptimizeGraph(rewriter_config, metagraph)
    metagraph.graph_def.CopyForm(optimized_graph)
